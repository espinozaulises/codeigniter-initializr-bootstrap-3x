...
class MY_Controller extends CI_Controller{
	...
	protected function _render_view($view, $return = FALSE) {
		//static
		...
		$merge = array_merge( $this->data, $toTpl );
		...
		if ( is_array($view) ) {
			...
			foreach ($view as $value) {
				$load .= $this->load->view($value, $merge, true);
			}
			$view = $load;
		}
		//data
		$toBody["content_body"] = $this->load->view($view, $merge,true);
		...
		//render view
		if( $return )
			return $this->load->view("template/skeleton",$toTpl, $return);
		else
